...
public class NGon implements IShape {
    ...
    public double square()
    {
        if (n > 3)
        {
            Point2D[] a = Arrays.copyOfRange(p,0,n-1);
    ...
    public double length()
    {
        ...
        Point2D prevp = p[0];
        for(Point2D point : p) {
            l += new Segment(prevp, point).length(); prevp = point;
        }
    ...
    public IShape shift(Point2D a) {
        for (int i = 0; i < n; i++) p[i].add(a);
        return this;
    }
    public IShape rot(double phi) {
        for (int i = 0; i < n; i++) p[i].rot(phi);
        return this;
    }
    public IShape symAxis(int i) {
        for (int j = 0; j < n; j++) p[j].symAxis(i);
        return this;
    }
    public boolean cross(IShape i)
    {
        ...
        return new Segment(p[0], p[n - 1]).cross(i);
    }
